 ...
 * 
 * TODO (rs2705): Create a more generalized/modularized setup where variables and methods related to specific
 * implementation types are self-contained in their own separate classes, instead of floating around in here.
 */
public final class JavaGitConfiguration {

    /*
     * The path to our git binaries. Default to null, which means that the git command is available via the system PATH
     ...
     * 
     * 1) When client code sets the path to the git binaries explicitly by calling {@link setGitPath}, this function is
     * used to determine if the path is usable or not. In this case, the <code>File</code> path argument will be the new
     * path to the git binaries.
     * 
     * 2) When client code calls {@link getGitVersion} and the path has not been set explicitly, we call this function
     * to figure out the version. In this case, the <code>File</code> path argument will be null.
     * 
     * 
     * @param path <code>File</code> object representing the directory containing the git binaries. If null, the
     *        previously-set path is used (if any). It must contain either an absolute path, or a valid path relative to
    ...
    private static String determineGitVersion(File path) throws JavaGitException {
        ...
        /*
         * If they already set the path explicitly, or are in the process of doing so (via the path argument), make sure
         * to prefix the git call with it. If they didn't, assume it's blank.
         */
        String gitPrefix = "";
        if (path != null) {
        ...
        try {
            ...
            response = (GitVersionResponse) ProcessUtilities.runCommand(null, commandLine, new GitVersionParser());
        } catch (Exception e) {
    ...
    /**
     * Return the complete string necessary to invoke git on the command line. Could be an absolute path to git, or if
     * the path was never set explicitly, just "git".
     * 
     * @return <code>String</code> containing the path to git, ending with the git executable's name itself.
     */
    ...
    /**
     * Return an absolute path capable of being dropped in front of the command-line git invocation. If the path hasn't
     * been set explicitly, just return the empty string and assume git is in this process' PATH environment variable.
     * 
    ...
    /**
     * Accessor method for the <code>File</code> object representing the path to git. If the git path is never set
     * explicitly, this will return null.
     * 
    ...
    /**
     * Returns the version number of the underlying git binaries. If this method is called and we don't know the version
     * yet, it tries to figure it out. (The version gets set if {@link #setGitPath(File) setGitPath} was previously
     * called.)
     * 
    ...
    /**
     * Judge the validity of a given git version string. This can be difficult to do, as there seems to be no
     * deliberately-defined git version format. So, here we do a minimal sanity check for two things: 1. The first
    ...
    private static boolean isValidVersionString(String version) {
        /*
         * Git version strings can vary, so let's do a minimal sanity check for two things: 1. The first character in
         * the version is a number. 2. There's at least one period in the version string.
         * 
         * TODO (rs2705): Make this more sophisticated by parsing out a major/minor version number, and ensuring it's >=
         * some minimally-required version.
         */
        try {
    ...
    /**
     * Called when client code wants to explicitly tell us where to find git on their filesystem. If never called, we
     * assume that git is in a directory in the PATH environment variable for this process. Passing null as the path
     * argument will unset an explicitly-set path and revert to looking for git in the PATH.
     * 
     * @param path <code>File</code> object representing the directory containing the git binaries. It must contain
     *        either an absolute path, or a valid path relative to the current working directory.
    ...
    public static void setGitPath(File path) throws IOException, JavaGitException {
        ...
        try {
            ...
            throw new JavaGitException(100002, ExceptionMessageMap.getMessage("100002") + " { path=[" + path.getPath()
                    + "] }", e);
        }
     ...
     * 
     * TODO (rs2705): Enforce the requirement below that the path be absolute. Is there a simple way to do this in an
     * OS-independent fashion?
     * 
     * @param path Absolute path to git binaries installed on the system. The path must be absolute since it needs to
     *        persist for the life of the client code, even if the working directory changes. Throws a
     *        NullPointerException if path is null, or an IllegalArgumentException if it's the empty string.
     * @throws IOException Thrown if the provided path does not exist.
    ...
    private static class GitVersionParser extends Parser {
        ...
        /**
         * Returns the <code>GitVersionResponse</code> object that's essentially just a wrapper around the git version
         * number.
         * 
