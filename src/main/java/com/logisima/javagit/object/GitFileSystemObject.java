...
public abstract class GitFileSystemObject {
     ...
     * 
     * @param in input <code>File</code> object repositoryPath path to git repository
     * @return <code>File</code> object with relative path
     * @throws JavaGitException input file does not belong to git working tree/ repo
     */
    ...
    public GitAddResponse add() throws IOException, JavaGitException {
        ...
        // run git-add command
        return gitAdd.add(workingTree.getPath(), list, null);
    }
     ...
     * 
     * @param dest destination path (relative to the Git Repository)
     * 
    ...
    public GitMvResponse mv(File dest) throws IOException, JavaGitException {
        ...
        GitMvResponse response = gitMv.mv(workingTree.getPath(), null, source, relativeDest);
    ...
    public GitRmResponse rm() throws IOException, JavaGitException {
        ...
        // run git rm command
        return gitRm.rm(workingTree.getPath(), null, relativePath);
    }
     ...
     * 
     * @param commit Git commit to compare with
     * 
