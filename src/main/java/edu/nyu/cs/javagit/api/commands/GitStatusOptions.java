 ...
 * 
 * The &lt;git-status&gt; command takes the same set of options as &lt;git-commit&gt; it shows what would be committed
 * if the same options are given to git-commit.
 */
...
public final class GitStatusOptions {
     ...
     * 
     * @return True if verbose(-v) option is passed to &lt;git-status&gt; command.
     */
     ...
     * 
     * @return True if read from log file(-F) is null.
     */
     ...
     * 
     * @param optReadFromLogFile <code>File</code> for reading log.
     */
    ...
    /**
     * optAll tells the command to automatically stage files that have been modified and deleted, but new files you have
    ...
    /**
     * Sets the '-a' options to &lt;git-status&gt; command. If any one of the optOnly or OptInclude options are set then
     * optAll should not be set. While setting optAll to true, if optOnly or optInclude is set set then
     ...
     * well. This is normally not used.
     * 
    ...
    /**
     * Sets the optOnly option for &lt;git-status&gt; command. optOnly should not be set if optAll(-a) option is set.
     * <code>IllegalArgumentException</code> is thrown if both '-a' and 'o' are set.
     * 
     * @param only
     */
    ...
    /**
     * Returns true if allow empty options is set. This option should be set if it's okay to record an empty change.
     * 
     * @return True if allow empty options is set. This option should be set if it's okay to record an empty change.
     */
    ...
    /**
     * Returns the author. This option should be used if we would like to override the value in commit.
     * 
     * @return author
     */
