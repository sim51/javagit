 ...
 * </ul>
 * 
 * Note: the last four digits may be divided into subtypes. In such case, the explanation of the format will be included
 * in the error type class descriptions below.<br>
 * <br>
 * 
 * The error types are:
 * 
 ...
 * 
 * <ul>
 * <li>All exceptions thrown from within javagit should contain a code and a message, even exceptions thrown using
 * standard Java exceptions.</li>
 * <li>When creating a new exception code and message, start the message with the code like so: "000001: "</li>
 * <li>When throwing a <code>JavaGitException</code>, the code and the message must be supplied to the constructor.</li>
 * <li>When throwing a standard Java exception (ex. <code>NullPointerException</code>), just supply the message since
 * the code is already included in the message.</li>
 * <li>The message retrieved from the <code>ExceptionMessageMap</code> can be treated like a base message. If additional
 * information is required in the message, append that information to the base message.</li>
 * <li>If the value of a variable must be included in the message, list the variable names and their values at the end
 * of the message in the following format: "{ varName1=[varVal1], varName2=[varVal2], ... }"</li>
 * </ul>
 * 
...
public class ExceptionMessageMap {
     ...
     * 
     * @param code The error code for which to get the associated error message.
