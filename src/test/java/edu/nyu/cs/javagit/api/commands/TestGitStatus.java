...
public class TestGitStatus extends TestCase {
    ...
    /**
     * Second Test for IOException being thrown where Repository directory does not exist and we try to create a file in
     * that directory.
     */
    ...
    /**
     * Test for new files that are created but not yet added by git-add or git-rm commands. These files are under
    ...
    public void testUntrackedNewFiles() throws IOException, JavaGitException {
        ...
        assertEquals("Error. Filename does not match.", "foobar01", response.getFileFromUntrackedFiles(0).getName());
        assertEquals("Error. Filename does not match.", "foobar02", response.getFileFromUntrackedFiles(1).getName());
        assertEquals("Error. Filename does not match.", "testDirectory", response.getFileFromUntrackedFiles(2)
                .getName());
    }
    ...
    public void testReadyToCommitNewFiles() throws IOException, JavaGitException {
        ...
        assertEquals("Error. Filename does not match", "foobar01", status.getFileFromNewFilesToCommit(0).getName());
        assertEquals("Error. Filename does not match", "foobar02", status.getFileFromNewFilesToCommit(1).getName());
        assertEquals("Error. Filename does not match", repositoryDirectory.getPath() + File.separator + "testDirectory"
                + File.separator + "foobar03", status.getFileFromNewFilesToCommit(2).toString());
    }

    /**
     * Test for files that are indexed and have been modified but git-add or git-rm command need to be run to get them
    ...
    public void testModifiedNotUpdatedFiles() throws IOException, JavaGitException {
        ...
        FileUtilities.modifyFileContents(new File(repositoryDirectory.getPath() + File.separator + file1.getPath()),
                "Test append text\n");
        FileUtilities.modifyFileContents(new File(testDir.getPath() + File.separator + file3.getPath()),
                "Another sample text added to foobar03\n");
        List<File> statusPath = null;
    ...
    /**
     * 
     @Test public void testModifiedFiles() throws IOException, JavaGitException { List<File> filesToAdd = new
     *       ArrayList<File>(); //File file4 = new File(testDir.getAbsoluteFile() + File.separator + "foobar04");
     *       //file4.createNewFile(); File file4 = FileUtilities.createFile(testDir, "foobar04", "Test File4");
     * 
     *       filesToAdd.add(file1); filesToAdd.add(file2); //filesToAdd.add(testDir); //filesToAdd.add(file3);
     *       filesToAdd.add(new File("testDirectory")); filesToAdd.add(new File( "testDirectory" + File.separator +
     *       file3.getPath())); //filesToAdd.add(file4); filesToAdd.add(new File( "testDirectory" + File.separator +
     *       file4.getPath()));
     * 
     *       GitAddOptions addOptions = new GitAddOptions(); // Add the files for committing
     *       gitAdd.add(repositoryDirectory, addOptions, filesToAdd); // Commit the added files
     *       gitCommit.commit(repositoryDirectory, "Test commit of two files"); // modify one of the committed files
     *       FileUtilities.modifyFileContents(new File(testDir.getPath() + File.separator + file3.getPath()),
     *       "Test append text\n"); FileUtilities.modifyFileContents(new File(testDir.getPath() + File.separator +
     *       file4.getPath()), "Another sample text added to foobar03\n"); File tmpFile = new File( testDir.getName() +
     *       File.separator + file3.getPath()); try { GitAddResponse addResponse = gitAdd.add(repositoryDirectory,
     *       addOptions, tmpFile); if ( addResponse.numberOfErrors() > 0 ) {
     *       System.out.println(addResponse.getError(0)); } GitStatusResponse response =
     *       gitStatus.status(repositoryDirectory, options); assertEquals(1, response.getModifiedFilesNotUpdatedSize());
     *       assertEquals("foobar04", response.getModifiedFilesNotUpdatedIterator().iterator().next().getName());
     *       assertEquals(1, response.getModifiedFilesToCommitSize()); assertEquals("foobar03",
     *       response.getModifiedFilesToCommitIterator().iterator().next().getName()); } catch ( JavaGitException e ) {
