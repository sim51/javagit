...
public class TestGitCommitResponse extends TestCase {
    ...
    public void testGitCommitResponseBasicFunctionality() {
        ...
        assertEquals("Short hash name is not the same.", Ref.createSha1Ref("3d3ef1a"), resp.getCommitShortHashName());
        ...
        assertEquals("Excpected different path to added file", new File("/some/path/to/file.txt"), adf.getPathTofile());
        assertEquals("Excpected different mode for added file", "100644", adf.getMode());
        adf = adIter.next();
        assertEquals("Excpected different path to added file", new File("/gumbee/fiddle/friars/whine.txt"),
                adf.getPathTofile());
        ...
        adf = adIter.next();
        assertEquals("Excpected different path to deleted file", new File("/some/path/to/deleted/file.txt"),
                adf.getPathTofile());
        assertEquals("Excpected different mode for deleted file", "100644", adf.getMode());
        adf = adIter.next();
        assertEquals("Excpected different path to deleted file", new File("/gumbee/fiddle/friars/del/whine.txt"),
                adf.getPathTofile());
        ...
        assertEquals("Excpected different path for to copied file", new File("/copied/path.txt"),
                cmf.getDestinationFilePath());
        ...
        assertEquals("Excpected different path for from copied file", new File("/filler/filet.txt"),
                cmf.getSourceFilePath());
        assertEquals("Excpected different path for to copied file", new File("/floppin/fandiggery.txt"),
                cmf.getDestinationFilePath());
        ...
        assertEquals("Excpected different path for from copied file", new File("/starter/path.txt"),
                cmf.getSourceFilePath());
        assertEquals("Excpected different path for to copied file", new File("/renamed/path.txt"),
                cmf.getDestinationFilePath());
        assertEquals("Excpected different percentage for copied file", 99, cmf.getPercentage());
        cmf = cmIter.next();
        assertEquals("Excpected different path for from copied file", new File("/filler/filet.txt"),
    ...
    public void testGitCommitResponseEqualsMethod() {
        ...
        resp.setFilesChanged(5);
        assertTrue("GitCommitResponseObjects are equal when they should not be.", !resp.equals(respSame));
        ...
        assertTrue("GitCommitResponseObjects are equal when they should not be.", !resp.equals(respSame));
        respSame.addCopiedFile(new File("/from/this/path.txt"), new File("/to/this/altered-path.txt"), 56);
        assertEquals("GitCommitResponse objects are not equal when they should be", resp, respSame);

        resp.addRenamedFile(new File("/from/another/path.txt"), new File("/to/another/altered-path.txt"), 23);
        assertTrue("GitCommitResponseObjects are equal when they should not be.", !resp.equals(respSame));
        respSame.addRenamedFile(new File("/from/another/path.txt"), new File("/to/another/altered-path.txt"), 23);
        assertEquals("GitCommitResponse objects are not equal when they should be", resp, respSame);
    ...
    public void testAddedOrDeletedFile() {
        GitCommitResponseImpl.AddedOrDeletedFile addDel = new AddedOrDeletedFile(
                new File("/a/path/to/add/del/file.txt"), "100644");
        ...
        assertEquals("AddedOrDeletedFile instances not equal when they should be equal", addDel, addDelSame);

        assertTrue("AddedOrDeletedFile instances equal when they should not be equal", !(addDel.equals(addDelDiff1)));
        assertTrue("AddedOrDeletedFile instances equal when they should not be equal", !addDel.equals(addDelDiff2));

        assertEquals("AddedOrDeletedFile hashcodes not equal when they should be equal.", addDel.hashCode(),
                addDelSame.hashCode());
    }

    @Test
    public void testCopiedOrMovedFile() {
        GitCommitResponseImpl.CopiedOrMovedFile copyMove = new CopiedOrMovedFile(new File("c:\\path\\1\\txt.txt"),
                new File("c:\\other\\path\\bob.txt"), 32);
        GitCommitResponseImpl.CopiedOrMovedFile copyMoveSame = new CopiedOrMovedFile(new File("c:\\path\\1\\txt.txt"),
                new File("c:\\other\\path\\bob.txt"), 32);
        ...
        GitCommitResponseImpl.CopiedOrMovedFile copyMoveDiff2 = new CopiedOrMovedFile(new File("c:\\path\\1\\txt.txt"),
                new File("c:\\path\\1\\bob.txt"), 32);
        GitCommitResponseImpl.CopiedOrMovedFile copyMoveDiff3 = new CopiedOrMovedFile(new File("c:\\path\\1\\txt.txt"),
                new File("c:\\other\\path\\bob.txt"), 83);

        assertEquals("CopiedOrMovedFile instances not equal when they should be equal", copyMove, copyMoveSame);

        assertTrue("CopiedOrMovedFile instances equal when they should not be equal", !copyMove.equals(copyMoveDiff1));
        assertTrue("CopiedOrMovedFile instances equal when they should not be equal", !copyMove.equals(copyMoveDiff2));
        assertTrue("CopiedOrMovedFile instances equal when they should not be equal", !copyMove.equals(copyMoveDiff3));

        assertEquals("CopiedOrMovedFile hashcodes not equal when they should be equal.", copyMove.hashCode(),
                copyMoveSame.hashCode());
    }
