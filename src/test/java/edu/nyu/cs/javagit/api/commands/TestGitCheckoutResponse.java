...
public class TestGitCheckoutResponse extends TestCase {
    ...
    /**
     * Test for confirming that <code>JavaGitException</code> is thrown while trying to checkout a non-existent branch
     * from the repository. Here the error flag should be set and the branch should be null as the given branch does not
     * exist.
     */
    @Test
    ...
    /**
     * Test for parsing fatal error generated while trying to create a new branch based on a non-existent base-branch.
     */
    @Test
    ...
    /**
     * Test for parsing the output line that contains the output with "Switched to branch" and the name of the branch in
     * double quotes. The output indicates that repository has been switched to an already existing another branch.
     */
    @Test
    ...
    /**
     * Test for parsing the output line that contains the output with "Switched to a new branch". The output indicates
     * that the new branch with the new name within quotes has been created and switched to.
     */
    @Test
    ...
    /**
     * Test for checking that <code>JavaGitException</code> is thrown when an invalid switch is passed to
     * &lt;git-checkout&gt; command. <code>GitCheckoutParser</code> should parse the error message properly and save it
     * into error object of <code>GitCheckoutResponse</code> object.
     */
    @Test
    ...
    /**
     * Test for checking the list of files that are added, modified or deleted but have not been committed to the branch
     * in repository.
     */
    @Test
    public void testGitCheckoutResponseAddedModifiedDeletedFiles() throws JavaGitException {
        ...
        assertEquals("First location - modifiedFileslist", "foobar01", response.getModifiedFile(0).getName());
        assertEquals("First location - deletedFileslist", "foobar03", response.getDeletedFile(0).getName());
