...
public class TestCheckUtilities extends TestCase {
    ...
    public void testCheckFileValidity() {
        ...
        if (file.exists()) {
            ...
            if (file.exists()) {
                assertTrue("In checking for file validity, the two test filenames are already in existance.", false);
            }
        }

        // Check for invalid case
        assertCheckFileValidityIOException(fullFilePath, "020001: File or path does not exist.  { filename=["
                + fullFilePath + "] }");
    ...
    private void assertCheckIntArgumentGreaterThanThrowsException(int toCheck, int lowerBound, String variableName,
        ...
        try {
            ...
            assertEquals("IllegalArgumentException didn't contain expected message.  Error!", expectedMessage,
                    e.getMessage());
        }
    }
    ...
    public void testCheckStringArgument() {
        ...
        assertCheckStringArgumentThrowsNPE(null, "aVariableName",
                "000001: A String argument was not specified but is required.  " + "{ variableName=[aVariableName] }");
    ...
    private void assertCheckStringArgumentThrowsIllegalArgException(String str, String variableName, String message) {
        try {
            CheckUtilities.checkStringArgument(str, variableName);
            assertTrue("IllegalArgumentException not thrown when one was expected!", false);
        } catch (IllegalArgumentException e) {
            assertEquals("IllegalArgumentException did not contain the expected message.", message, e.getMessage());
        }
    }
    ...
    private void assertCheckStringListArgumentThrowsNPE(List<String> l, String variableName, String message) {
        try {
            CheckUtilities.checkStringListArgument(l, variableName);
            assertTrue("NPE exception not thrown when one was expected!", false);
        } catch (NullPointerException e) {
    ...
    private void assertCheckStringListArgumentThrowsIllegalArgException(List<String> l, String variableName,
            String message) {
        try {
            CheckUtilities.checkStringListArgument(l, variableName);
            assertTrue("IllegalArgumentException not thrown when one was expected!", false);
