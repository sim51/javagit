...
public class TestCliGitCommit extends TestCase {
    ...
    public void testGitCommitParserValidInput() {
        ...
        response.addRenamedFile(new File("testing_idr/hildebrand.bob"), new File("hildebrand.bob"), 100);
        response.addRenamedFile(new File("svnClientAdapter/changelog.txt"), new File("svnClientAdapter/CHlog.txt"), 100);
        assertResponsesEqual(parser, response);
    ...
    private void assertResponsesEqual(CliGitCommit.GitCommitParser parser, GitCommitResponseImpl response) {
        try {
            assertTrue("Expected GitCommitResponse not equal to actual GitCommitResponse.",
    ...
    private void assertExceptionThrownOnResponseRetrieval(CliGitCommit.GitCommitParser parser, String message, int code) {
        try {
            parser.getResponse();
            assertTrue("Got GitCommitResponse from GitCommitParser when an exception should have been thrown!", false);
        } catch (JavaGitException e) {
