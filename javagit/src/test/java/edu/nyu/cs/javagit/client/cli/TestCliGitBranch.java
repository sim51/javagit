...
public class TestCliGitBranch extends TestCase {
    ...
    public void testGitBranchParserValidInput() {
        ...
        response = new GitBranchResponseImpl();
        BranchRecord record = new BranchRecord(Ref.createBranchRef("doc-refactor"), Ref.createSha1Ref("2a66ab6"),
                "Committing a document", false);
        response.addIntoBranchList(Ref.createBranchRef("doc-refactor"));
        response.addIntoListOfBranchRecord(record);
        record = new BranchRecord(Ref.createBranchRef("master"), Ref.createSha1Ref("23d48c1"), "new file", false);
        response.addIntoBranchList(Ref.createBranchRef("master"));
        response.addIntoListOfBranchRecord(record);
        ...
        response.addIntoBranchList(Ref.createBranchRef("refactor"));
        response.addIntoListOfBranchRecord(record);
        record = new BranchRecord(Ref.createBranchRef("xyz"), Ref.createSha1Ref("03529b8"), "commit", true);
        response.addIntoBranchList(Ref.createBranchRef("xyz"));
        response.addIntoListOfBranchRecord(record);
        response.setCurrentBranch(Ref.createBranchRef("xyz"));
        response.setResponseType(responseType.BRANCH_LIST);
    ...
    private void assertResponsesEqual(CliGitBranch.GitBranchParser parser, GitBranchResponseImpl response) {
        try {
            assertTrue("Expected GitBranchResponse not equal to actual GitBranchResponse.",
                    response.equals(parser.getResponse()));
    ...
    private void assertExceptionThrownOnResponseRetrieval(CliGitBranch.GitBranchParser parser, String message, int code) {
        try {
            parser.getResponse();
            assertTrue("Got GitBranchResponse from GitBranchParser when an exception should have been thrown!", false);
        } catch (JavaGitException e) {
            assertTrue("Exception message is not correct for thrown exception.", e.getMessage().equals(message));
