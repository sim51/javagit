...
public class TestGitStatusResponse extends TestCase {
    ...
    /**
     * Test for JavaGitException is thrown when an invalid switch is passed to &lt;git-status&gt;
     */
    @Test
    ...
    /**
     * Test for JavaGitException is thrown if an invalid file name is passed to &lt;git-status&gt; command.
     */
    @Test
    ...
    /**
     * Test for confirming that lines filenames are parsed correctly and correct filenames are extracted from the line.
     */
    @Test
    ...
    /**
     * Test for verifying that correct no. of untracked files are created in the <code>GitStatusResponse</code> object
     * when <code>GitStatus</code> is run.
     */
    @Test
    ...
    public void testNewlyAddedFiles() throws JavaGitException {
        ...
        assertEquals("File Name", repositoryDirectory + "dir" + File.separator + "fileD", iter.next().getPath());
    ...
    /**
     * Test simulates parsing an output generated by &lt;git-status&gt; command and verifies that no. of files parsed
     * for newFilesToCommit, deletedFilesToCommit, deletedFilesNotUpdated and modifiedFilesNotUpdated are correct and
     * match.
     */
    @Test
    public void testModifiedAndDeletedFiles() throws JavaGitException {
        ...
        assertEquals("FileName", repositoryDirectory + "dir" + File.separator + "fileD", response.getNewFilesToCommit()
                .iterator().next().getPath());
        assertEquals("FileName", repositoryDirectory + "fileC", response.getDeletedFilesToCommit().iterator().next()
                .getPath());
        assertEquals("FileName", repositoryDirectory + "fileA", response.getModifiedFilesNotUpdated().iterator().next()
                .getPath());
        assertEquals("FileName", repositoryDirectory + "fileB", response.getDeletedFilesNotUpdated().iterator().next()
                .getPath());
    }
    ...
    public void testRenamedFilesToCommitFromCommandOutput() throws IOException, JavaGitException {
        ...
        assertEquals("Renamed File name does not match", repositoryDirectory + "file1", renamedFiles.iterator().next()
                .getPath());
        assertEquals("Renamed File name does not match", repositoryDirectory + "file6", renamedFiles.iterator().next()
                .getPath());
        assertEquals("No. of renamed files", 2, response.getRenamedFilesToCommitSize());
    }

    /**
     * Test for verifying that modified staged and un-staged files are parsed properly by <code>GitStatusParser</code>
     */
    @Test
    public void testModifiedFilesFromCommandOutput() throws IOException, JavaGitException {
        ...
        assertEquals("ModifiedNotStaged.java", response.getModifiedFilesNotUpdated().iterator().next().getName());
