...
public class TestCliGitAdd extends TestCase {
    ...
    /**
     * Test for parsing the a valid output generated after a couple of files are added to the repository. The output
     * starts with 'add'.
     */
    @Test
    public void testGitAddParserValidInput() throws JavaGitException {
        ...
        assertEquals("File path provided when adding does not match.", "test-repository" + File.separator + "eg9",
                response.get(0).toString());
        assertEquals("File name does not match", "foo", response.get(1).getName());
        assertEquals("File path provided when adding does not match.", "test-repository" + File.separator + "testdir"
                + File.separator + "foo", response.get(1).toString());
    }

    /**
     * This is a test for adding a file to the git repository that is non-existing. Test verifies that error condition
     * is raised in the <code>GitAddResponse</code> and error string is saved.
     * <p>
     * e.g. git-add ./foobar001 fatal: pathspec 'test-repository/foobar001' did not match any files
     * <p>
     * In the above example the file foobar001 is file that is not physically existing inside the repository and this
     * test parsed the error message generated by the git-add command.
     */
    @Test
