...
public class GitBranchOptions {
    ...
    /*
     * The --no-color option. Turn off branch colors, even when the configuration file gives thedefault to color output.
     */
     ...
     * 
     * @return Ref Ref if the verbose option should be used, null otherwise.
     */
    ...
    public void checkCanSetNoArgOption(String option) {
        if (isOptTrack() || isOptNoTrack() || isOptL() || isOptF() || isOptDLower() || isOptDUpper() || isOptMLower()
                || isOptMUpper()) {
            throw new IllegalArgumentException(ExceptionMessageMap.getMessage("000120") + option
                    + " should be used without arguments, to display branches");
        }
    }
    ...
    public void checkCanSetCreateOption(String option) {
        if (isOptColor() || isOptNoColor() || isOptR() || isOptA() || isOptVerbose() || isOptMerged()
                || isOptNoMerged() || (null != getOptContains()) || isOptMLower() || isOptMUpper() || isOptDLower()
                || isOptDUpper()) {
            throw new IllegalArgumentException(ExceptionMessageMap.getMessage("000120") + option
    ...
    public void checkCanSetDeleteOption(String option) {
        if (isOptColor() || isOptNoColor() || isOptA() || isOptVerbose() || isOptMerged() || isOptNoMerged()
                || (null != getOptContains()) || isOptTrack() || isOptNoTrack() || isOptL() || isOptF()
    ...
    public void checkCanSetRenameOption(String option) {
        if (isOptColor() || isOptNoColor() || isOptR() || isOptA() || isOptVerbose() || isOptMerged()
                || isOptNoMerged() || (null != getOptContains()) || isOptTrack() || isOptNoTrack() || isOptL()
                || isOptF() || isOptDLower() || isOptDUpper()) {
            throw new IllegalArgumentException(ExceptionMessageMap.getMessage("000120") + option
